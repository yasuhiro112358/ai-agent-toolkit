...
既存のクラス図・コールグラフを VS Code でプレビューし、任意のシナリオのシーケンス図をオンデマンドで派生させる。`pack.py` 不要。

### 管理者（図を作る・維持する）

コード変更のたびに `pack.py` でソースをバンドルし、AI にプロンプトと合わせて貼り付けてマスタ図を更新する。9 言語対応（C/C++・Java・C#・Python・TypeScript・JavaScript・PHP・Rust・Visual Basic）。

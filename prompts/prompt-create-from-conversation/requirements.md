...
sequenceDiagram
    ...
    Note over U,A: ステップ1
    U->>A: requirements-create-from-conversation.md を同一セッションの末尾に貼り付け
    A->>A: 要件定義書を作成
    A->>A: 自己チェック
    A->>A: 問題があれば修正
    A->>U: 要件定義書を出力
    U->>U: 要件定義書をレビュー・修正
    Note over U,A: ステップ2
    U->>A: 要件定義書 + prompt-create-from-requirements.md を貼り付け
    A->>A: 汎用プロンプトを作成
    A->>A: 自己チェック
    A->>A: 問題があれば修正
    A->>U: 汎用プロンプトを出力
